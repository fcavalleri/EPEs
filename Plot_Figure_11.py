...
#breaks = np.linspace(0,400,21)
breaks = np.linspace(1,31, 11)
...
for season in seasons:
    ...
    cs = axs[seasons.index(season)].contourf(xx,  yy, 
            clim_val.reshape(xx.shape).T, cmap="BuPu",extend='max',levels=breaks)
...
cbar.set_label("N (EPEs/season)", fontsize=20, rotation=270, labelpad=20)
