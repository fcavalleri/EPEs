...
udms = ["#events","mm/h", "km", "mm/h", "eccentricity",""]
...
breaks = np.arange(0,200, 15)
#breaks = np.linspace(12,60,17)

print(var)
# same figure with 4 seasons
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()},figsize=(15, 12))
...
for season in seasons:
    ...
    axs[seasons.index(season)].set_extent([min_lon+0.5, max_lon, min_lat+1, max_lat])
...
cbar.set_label("EPEs SpS (km)", fontsize=20, rotation=270, labelpad=20)
cbar.ax.set_aspect(8)
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=20)
plt.savefig(path_out +'pctl_EXT_Climatology_' + var + '_seas.pdf')
plt.close()
