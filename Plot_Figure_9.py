...
##### COUNTS
var = 'counts'
breaks = np.linspace(0,320,  17)

print(var)
# same figure with 4 seasons
fig, axs = plt.subplots(2, 2, subplot_kw={'projection': ccrs.PlateCarree()},figsize=(15.2, 12))
...
for season in seasons:
    ...
    cs = axs[seasons.index(season)].contourf(xx,  yy, 
            clim_val.reshape(xx.shape).T, cmap="BuPu",levels=breaks,extend='max')
    # add season as bottom left box
...
plt.tight_layout()
cbar = fig.colorbar(cs, ax=axs, orientation='vertical', fraction=0.09, pad=0.025)#,ticks=breaks)
cbar.set_label('N (events/season)', fontsize=20, rotation=270, labelpad=20)
...
var = 'freq_wet'
##### to be setted breaks for each var
#breaks = np.linspace(0,1, 11)
breaks = np.linspace(0,0.1, 11)
print(var)
# same figure with 4 seasons
...
for season in seasons:
    ...
    # plot seasonal field with pcolormesh
    cs = axs[seasons.index(season)].contourf(xx,  yy,
            clim_val.reshape(xx.shape).T, cmap="YlGn",levels=breaks,extend='max')
                # tot tp norm: cmap="winter_r", levels=np.linspace(1.5,4, 6),extend='both')

                #generic: vmin=vmins[vars_to_clim.index(var)],vmax=vmaxs[vars_to_clim.index(var)])
    ...
    axs[seasons.index(season)].text(0.05, 0.05, season, fontsize=20, transform=axs[seasons.index(season)].transAxes,
            ...
            # add season as bottom left box
plt.tight_layout()
# set a lateral space on the right for the colorbar
cbar = fig.colorbar(cs, ax=axs, orientation='vertical', fraction=0.09, pad=0.03,ticks=breaks)
cbar.set_label('hours with events', fontsize=20, rotation=270, labelpad=20)
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=15)
cbar.ax.set_yticklabels(['{:,.0%}'.format(x) for x in breaks])
plt.savefig(path_out + 'pctl_climatology_' + var + '_seas.png')
plt.close() 
...
##### to be setted breaks for each var
#breaks = np.linspace(0,210, 22)
breaks = np.arange(10,80,5)
...
for season in seasons:
    ...
    #cs = axs[seasons.index(season)].contourf(lons.reshape(xx.shape), lats.reshape(xx.shape), 
                #slopes.reshape(xx.shape).T, cmap='rainbow', levels=100,extend='both')  #levels=np.linspace(50, 400, 8)
    cs = axs[seasons.index(season)].contourf(xx,  yy, 
            clim_val.reshape(xx.shape).T, cmap="gnuplot2_r",levels=breaks,extend='max')
                # tot tp norm: cmap="winter_r", levels=np.linspace(1.5,4, 6),extend='both')
                
                #generic: vmin=vmins[vars_to_clim.index(var)],vmax=vmaxs[vars_to_clim.index(var)])             
    # add individual colorbar for the subplot
    #cbar = fig.colorbar(cs, ax=axs[seasons.index(season)], orientation='vertical', fraction=0.046, pad=0.04) #ticks=range(0, 225, 25)
    #cbar.set_label('km', fontsize=20, rotation=270, labelpad=20)
    # set season as title
    axs[seasons.index(season)].text(0.05, 0.05, season, fontsize=20, transform=axs[seasons.index(season)].transAxes,
            bbox=dict(facecolor='white', alpha=0.5, edgecolor='black', boxstyle='round,pad=0.3'))
...
plt.tight_layout()
cbar = fig.colorbar(cs, ax=axs, orientation='vertical', fraction=0.09, pad=0.03)#,ticks=breaks)
cbar.set_label("SpS (km)", fontsize=20, rotation=270, labelpad=20)
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=20)
plt.savefig(path_out + 'pctl_climatology_' + var + '_seas.pdf')
plt.close()
...
var = 'tp_max'
##### to be setted breaks for each var
breaks = np.arange(1,26,2)
#breaks = np.linspace(20,60, 9)
...
for season in seasons:
    ...
    #cs = axs[seasons.index(season)].contourf(lons.reshape(xx.shape), lats.reshape(xx.shape), 
                #slopes.reshape(xx.shape).T, cmap='rainbow', levels=100,extend='both')  #levels=np.linspace(50, 400, 8)
    cs = axs[seasons.index(season)].contourf(xx,  yy, 
            clim_val.reshape(xx.shape).T, cmap="gist_stern_r",levels=breaks,extend='max')
                # tot tp norm: cmap="winter_r", levels=np.linspace(1.5,4, 6),extend='both')
...
plt.tight_layout()
cbar = fig.colorbar(cs, ax=axs, orientation='vertical', fraction=0.09, pad=0.03,ticks=breaks)
cbar.set_label("PkIn mm/h", fontsize=20, rotation=270, labelpad=20)
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=20)
plt.savefig(path_out + 'pctl_climatology_' + var + '_seas.pdf')
plt.close()
...
var = "eccentricity"
##### to be setted breaks for each var
breaks = np.linspace(0.65,0.85,5)
print(var)

fig, axs = plt.subplots(2, 2, subplot_kw={'projection': ccrs.PlateCarree()},figsize=(16, 12))
#fig.suptitle('Seasonal climatology of '+ var, fontsize=35)
axs = axs.ravel()
for season in seasons:  
    #load matrix saved as txt at df_counts_avg.to_csv(path_out + 'EXT_eachyr_Seas_EventBased_Mean_counts_' + seas + '.txt', sep=' ', index=False)
    ...
    # plot seasonal field with pcolormesh
    cs = axs[seasons.index(season)].contourf(xx,  yy, 
            clim_val.reshape(xx.shape).T, cmap="YlOrRd",levels=breaks,extend='max')
                # tot tp norm: cmap="winter_r", levels=np.linspace(1.5,4, 6),extend='both')
...
plt.tight_layout()
cbar = fig.colorbar(cs, ax=axs, orientation='vertical', fraction=0.09, pad=0.02,ticks=breaks)
cbar.set_label(udms[vars_to_clim.index(var)], fontsize=20, rotation=270, labelpad=20)
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=20)
plt.savefig(path_out + 'pctl_climatology_' + var + '_seas.png')
plt.close()
...
for season in seasons:
    #load matrix saved as txt at df_counts_avg.to_csv(path_out + 'EXT_eachyr_Seas_EventBased_Mean_counts_' + seas + '.txt', sep=' ', index=False)
    mat_seas = pd. read_csv(path_in + 'EXT_eachyr_Seas_EventBased_Mean_' + var + '_' + season + '.txt', sep=' ')
    # avg each row exluded lon and lat
    clim_val = np.nanmean(mat_seas.iloc[:, 2:], axis=1)
...
cbar.ax.xaxis.set_label_position('top')
cbar.ax.tick_params(labelsize=20)
plt.savefig(path_out + 'EXT_Climatology_' + var + '_seas.png')
plt.close()
